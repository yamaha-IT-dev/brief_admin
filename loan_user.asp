...
sub displayLoanStock
	...
	strSQL = strSQL & " 	stockLocation, stockRenewalCounter, stockID, stockDateCreated, stockCreatedBy, stockDateModified, stockModifiedBy, "
	...
	strSQL = strSQL & "		LEFT JOIN (SELECT max(renApprovalDate) as renApprovalDate2, renOrderNo, renOrderLine FROM tbl_loan_renewal GROUP BY renOrderNo, renOrderLine) AS RENEWAL ON order_no = renOrderNo AND order_line = renOrderLine "
	strSQL = strSQL & "		LEFT JOIN tbl_loan_renewal AS LN ON RENEWAL.renOrderNo = LN.renOrderNo and RENEWAL.renOrderLine = LN.renOrderLine and LN.renApprovalDate = RENEWAL.renApprovalDate2 "
	strSQL = strSQL & "		LEFT JOIN tbl_loan_transfer ON traOrderNo = order_no AND traOrderLine = order_line "
	strSQL = strSQL & "		LEFT JOIN tbl_loan_sale ON saleOrderNo = order_no AND saleOrderLine = order_line "
	...
	if not DB_RecSetIsEmpty(rs) Then	
        ...
        strDisplayList = "<tr><td colspan=""14"" align=""center"">No stocks found.</td></tr>"
	end if
	strDisplayList = strDisplayList & "<tr>"
