...
sub displayOutstandingInvoices
	...
	strSQL = strSQL & " INNER JOIN (SELECT DISTINCT G1UKNO, CASE WHEN G1TSYK = 1 THEN '' ELSE B6AHNO END AS B6AHNO, G1TSYK FROM GF1EP LEFT JOIN BF6EP ON G1UKNO = B6INNO GROUP BY G1UKNO, G1UKGN, B6AHNO, G1TSYK) A on B.G1UKNO = A.G1UKNO AND B.G1TSYK = A.G1TSYK "
