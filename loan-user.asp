...
sub displayLoanStock
    ...
    strSQL = strSQL & "SELECT DISTINCT order_no, order_line, set_sequence, department, account_code, account_code_ext, account_name, item_code, serial_no,  "
    strSQL = strSQL & "		B9AHEN, B9SKSU, lic, qty, loan_year, loan_month, loan_day, loan_date, "
    strSQL = strSQL & " 	stockLocation, stockSetSequence, stockRenewalCounter, stockID, stockDateCreated, stockCreatedBy, stockDateModified, stockModifiedBy, "
    strSQL = strSQL & " 	traModelNo, traStatus, saleModelNo, saleStatus, product_code,renStatus, renActive, renExpiryDate, renDateCreated, renApprovalDate, renModifiedBy, renComments"
    strSQL = strSQL & " FROM OPENQUERY "
    strSQL = strSQL & " (AS400, 'SELECT B9JUNO AS order_no, B9JUGY AS order_line, B9STJN as set_sequence, Y1REGN AS department, B9URKC AS account_code, B9JURC AS account_code_ext,  "
    ...
    strSQL = strSQL & "	')"
    strSQL = strSQL & "		LEFT JOIN tbl_loan_location ON stockOrderNo = order_no AND stockOrderLine = order_line AND stockSetSequence = set_sequence  "
    strSQL = strSQL & "		LEFT JOIN (SELECT max(renApprovalDate) as renApprovalDate2, renOrderNo, renOrderLine, renserialNo FROM tbl_loan_renewal GROUP BY renOrderNo, renOrderLine, renserialno) AS RENEWAL ON order_no = renOrderNo AND order_line = renOrderLine  and renSerialNo = serial_no "
    strSQL = strSQL & "		LEFT JOIN tbl_loan_renewal AS LN ON RENEWAL.renOrderNo = LN.renOrderNo and RENEWAL.renOrderLine = LN.renOrderLine and LN.renApprovalDate = RENEWAL.renApprovalDate2  AND RENEWAL.renSerialNo = serial_no "
    strSQL = strSQL & "		LEFT JOIN tbl_loan_transfer ON traOrderNo = order_no AND traOrderLine = order_line AND traSerialNo = serial_no "
    strSQL = strSQL & "		LEFT JOIN tbl_loan_sale ON saleOrderNo = order_no AND saleOrderLine = order_line AND saleSerialNo = serial_no "
    strSQL = strSQL & "		LEFT JOIN Workflow.dbo.workflow_loan_return_item_list ON Workflow.dbo.workflow_loan_return_item_list.order_number = order_no AND Workflow.dbo.workflow_loan_return_item_list.order_lines = order_line "
    ...
    if not DB_RecSetIsEmpty(rs) Then
        ...
        For intRecord = 1 To rs.PageSize
            ...
            if IsNull(rs("traModelNo")) or rs("traStatus") = 2 then
                strDisplayList = strDisplayList & "	<form method=""post"" name=""form_auction"" id=""form_auction"" onsubmit=""return submitTransferForm(this)"">"
                strDisplayList = strDisplayList & "		<input type=""hidden"" name=""action"" value=""Transfer"">"
            ...
            strDisplayList = strDisplayList & "</td>"
            strDisplayList = strDisplayList & "<td>"
            if IsNull(rs("saleModelNo")) or rs("saleStatus") = 2 then
                strDisplayList = strDisplayList & "	<form method=""post"" name=""form_auction"" id=""form_auction"" onsubmit=""return submitSaleForm(this)"">"
                strDisplayList = strDisplayList & "		<input type=""hidden"" name=""action"" value=""Sale"">"
                strDisplayList = strDisplayList & "		<input type=""hidden"" name=""account_code"" value=""" & Trim(rs("account_code")) & """>"
                strDisplayList = strDisplayList & "		<input type=""hidden"" name=""item_code"" value=""" & Trim(rs("item_code")) & """>"
                strDisplayList = strDisplayList & "		<input type=""hidden"" name=""serial_no"" value=""" & Trim(rs("serial_no")) & """>"
                strDisplayList = strDisplayList & "		<input type=""hidden"" name=""order_no"" value=""" & Trim(rs("order_no")) & """>"
                strDisplayList = strDisplayList & "		<input type=""hidden"" name=""order_line"" value=""" & Trim(rs("order_line")) & """>"
                strDisplayList = strDisplayList & "		<input type=""submit"" value=""Sale"" class=""btn btn-warning"" />"
                strDisplayList = strDisplayList & "	</form>"
            else
            ...
            '    strDisplayList = strDisplayList & "		<input type=""hidden"" name=""order_no"" value=""" & Trim(rs("order_no")) & """>"
            '    strDisplayList = strDisplayList & "		<input type=""hidden"" name=""order_line"" value=""" & Trim(rs("order_line")) & """>"
            '    strDisplayList = strDisplayList & "		<input type=""submit"" value=""Add to workflow"" class=""btn btn-success"" />"
            '    strDisplayList = strDisplayList & "	</form>"
            'else
            ...
            if intDays > 85 and IsNull(rs("product_code")) then
                ...
                strDisplayList = strDisplayList & "		<input type=""submit"" value=""Renew"" class=""btn btn-danger"" />"
                strDisplayList = strDisplayList & "	</form>"
                'end if
            else
            ...
            Select Case	rs("renStatus")
                case 1
                    strDisplayList = strDisplayList & "<font color=""blue""><em>Pending Approval</em></font>"
                case 2
                    strDisplayList = strDisplayList & "<font color=""red""><i class=""fa fa-close""></i> Rejected </font><br/><strong>" & rs("renModifiedBy") & "</strong><br/>" & rs("renApprovalDate") & ""
                case 0
                    strDisplayList = strDisplayList & "<font color=""green""><i class=""fa fa-check-square-o""></i> Approved </font><br/><strong>" & rs("renModifiedBy") & "</strong><br/>" & rs("renApprovalDate") & ""
            end select
            strDisplayList = strDisplayList & "</td>"
            strDisplayList = strDisplayList & "<td>" & rs("renComments") & "</td>"
            strDisplayList = strDisplayList & "<td align=""left"">"
            ...
            if IsNull(rs("stockLocation")) then
                ...
                strDisplayList = strDisplayList & "                 <input type=""hidden"" name=""order_no"" value=""" & Trim(rs("order_no")) & """>"
                strDisplayList = strDisplayList & "                 <input type=""hidden"" name=""order_line"" value=""" & Trim(rs("order_line")) & """>"
                strDisplayList = strDisplayList & "                 <input type=""hidden"" name=""stock_set_sequence"" value=""" & Trim(rs("stockSetSequence")) & """>"
                ...
                strDisplayList = strDisplayList & " <form method=""post"" name=""form_update_location"" id=""form_update_location"" onsubmit=""return validateUpdateLocation(this)"">"
                strDisplayList = strDisplayList & "     <table>"
                strDisplayList = strDisplayList & "         <tr>"
                strDisplayList = strDisplayList & "             <td align=""center"">"
        ...
        strDisplayList = "<tr><td colspan=""10"" align=""center"">No stocks found.</td></tr>"
    end if
    strDisplayList = strDisplayList & "<tr>"
    strDisplayList = strDisplayList & "<td colspan=""15"" align=""center"">"
...
sub main
    ...
    strStockSetSequence = Request("stock_set_sequence")
    ...
    select case Trim(Request("Action"))
        ...
        case "Sale"
            call newLoanSale(strAccountCode, strItemCode, strSerialNo, strOrderNo, strOrderLine)
        case "Add"
            call addLoanLocation(strOrderNo, strOrderLine, strStockSetSequence, strLocation, session("logged_username"))
        case "Update"
...
%>
    ...
    <div class="container">
        ...
        <h1 class="page-header"><i class="fa fa-search-plus"></i> <%= session("loan_user_account") & "-" & session("account_code_ext") & " " & session("loan_user_account_name") & "" %></h1>
    ...
    <p align="center">
        <a href="export_loan_user.asp?search=<%= request("txtSearch") %>&account=<%= session("loan_user_account") %>&year=<%= session("loan_user_year") %>&month=<%= session("loan_user_month") %>&sort=<%= session("loan_user_sort") %>">
            <button type="button" class="btn btn-success"><i class="fa fa-download"></i> Export</button>
        </a>
        <a href="export_loan_user_location.asp?account=<%= session("loan_user_account") %>">
            <button type="button" class="btn btn-success"><i class="fa fa-download"></i> Export with Location</button>
        </a>
    </p>
    <p align="center">
        <small><%= session("emp_department") %> - Logged in as: <%= session("logged_username") %> (<%= UCASE(trim(session("emp_initial"))) %>) - Admin: <%= session("emp_admin") %></small>
    </p>
